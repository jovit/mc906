...
class RobotProblem(Problem):
    ...
    def actions(self, state):
        ...
        actions = [Action.ROTATE_LEFT, Action.ROTATE_RIGHT]
        if (state.angle == 0 and self.maze[state.x, state.y + 1] != MazePositionType.WALL) or \
           (state.angle == 90 and self.maze[state.x + 1, state.y] != MazePositionType.WALL) or \
